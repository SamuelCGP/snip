...
func onDamageEffect(_source: CardNode, targets: Array[CardData], amount: int):
...
func setCardCanBattle(card: CardNode, value: bool):
	if card.state == CardNode.CardState.SUMMONED:
		if card.playerOwner.isTurnPlayer && card.cardData.function == CardData.Function.CREATURE:
			if card.cardData.attack:
				card.canAttack = value
		elif (
			card.cardData.function == CardData.Function.MAGE
			|| card.cardData.function == CardData.Function.CREATURE
		):
...
func onCardClick(card: CardNode):
	if (
		card.playerOwner.id != PlayerManager.PlayerID.ALLY
		&& attackerCard
		&& !targetCard
		&& card.canBeTargeted
	):
...
func onCardDestroyed(card: CardNode):
	if card.cardData.function == CardData.Function.MAGE:
		print("Game Over " + str(PlayerManager.PlayerID.find_key(card.playerOwner.id)))
	else:
